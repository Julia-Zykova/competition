...
from drf.schemas import invalid_inputs, list_photos
from drf.serializers import CommentSerializer, PhotoBaseSerializer, PhotoSerializer
...
class ListCreatePhotosAPIView(generics.ListCreateAPIView):
    ...
    def post(self, request: HttpRequest, *args, **kwargs) -> "HttpResponse":
        outcome = ServiceOutcome(
            UploadPhotoService,
            self.request.POST.dict() | {"author": self.request.user if self.request.user.is_authenticated else None},
...
class RetrieveUpdateDestroyPhotoAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    @swagger_auto_schema(
        responses={
            "204": openapi.Response("Photo was marked as deleted successfully", schema=PhotoSerializer),
            "400": "Invalid parameters",
    ...
    def delete(self, request: HttpRequest, *args, **kwargs) -> "HttpResponse":
        outcome = ServiceOutcome(SoftDeletePhotoService, request.data | {"photo": self.kwargs["photo"]})
    ...
    def patch(self, request: HttpRequest, *args, **kwargs) -> "HttpResponse":
        outcome = ServiceOutcome(EditPhotoService, request.data | {"photo": self.kwargs["photo"]})
        serializer = PhotoSerializer(outcome.result)
...
class UpdatePhotoRestoreAPIView(generics.UpdateAPIView):
    ...
    def patch(self, request: HttpRequest, *args, **kwargs) -> "HttpResponse":
        outcome = ServiceOutcome(RestorePhotoService, request.data | {"photo": self.kwargs["photo"]})
        serializer = PhotoSerializer(outcome.result)
        return Response(serializer.data, status=status.HTTP_200_OK)
