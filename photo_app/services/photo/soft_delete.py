...
class SoftDeletePhotoService(ServiceWithResult):
    ...
    def _soft_delete(self):
        ...
        for user in users_set:
            async_to_sync(channel_layer.group_send)("user_" + str(user), {"type": "user.message", "message": message})
        import environ

        env = environ.Env()
        delete_photo.apply_async(args=[self.cleaned_data["photo"]], countdown=int(env("TIME_BEFORE_DELETE")))
