...
class CommentView(View):
    ...
    def post(self, request, **kwargs):
        if request.POST["_method"] == "POST":
            ServiceOutcome(
                CommentForPhotoService,
                request.POST.dict()
                | {"user": request.user if self.request.user.is_authenticated else None, "photo": self.kwargs["photo"]},
            )

        if request.POST["_method"] == "PATCH":
            ServiceOutcome(EditCommentService, request.POST.dict())

        elif request.POST["_method"] == "DELETE":
            ServiceOutcome(DeleteCommentService, request.POST.dict())
