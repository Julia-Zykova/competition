...
from .models import Comment, CustomUser, Photo, Voice
...
class UserAdmin(admin.ModelAdmin):
    ...
    def personal_photo(obj):
        return mark_safe(f'<img src="{obj.user_photo.url}">')  # nosec B703, B308
...
class PhotoAdminForm(forms.ModelForm):
    ...
    def clean_state(self):
        ...
        if self.initial["state"] == "approved" and self.cleaned_data["state"] == "rejected":
        ...
        elif self.initial["state"] == "on_delete" and self.cleaned_data["state"] in [
            "in_moderation",
            "approved",
            "rejected",
        ]:
            ...
            reject_photo.apply_async(args=[self.instance.id], countdown=int(env("TIME_BEFORE_REJECT")))
...
class PhotoAdmin(admin.ModelAdmin):
    ...
    def save_form(self, request, form, change):
        if form.instance.state == "rejected":
            ...
            env = environ.Env()
            reject_photo.apply_async(args=[form.instance.id], countdown=int(env("TIME_BEFORE_REJECT")))

            form.instance.state = "in_moderation"
        return form.save(commit=False)

    def post_photo(self, obj):
        return mark_safe(f'<img src="{obj.photo_small.url}">')  # nosec B703, B308
    ...
    inlines = [VoiceInline, CommentInline]
